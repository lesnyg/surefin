...
//    private Recipient recipient;
    ...
    public void servicelistQuery() {
        try {
            ...
            ResultSet yearRS = statement.executeQuery("select 금액 from Su_년도별적용급액 where 년도='" + thisYear + "' and 상세구분='" + recipientBasicTime + "'");
            while (yearRS.next()) {
                String yearsupport = yearRS.getString("금액");
                intyearsupport = Integer.parseInt(yearsupport);
            }

            ResultSet bathPriceRS = statement.executeQuery("select A.목욕여부,B.금액 from Su_방문목욕정보 as A LEFT JOIN Su_년도별적용급액 as B ON A.차량이용=B.상세구분 where A.수급자코드='" + recipiId + "' AND (A.일자 BETWEEN '" + startMon + "' AND '" + endMon + "') AND B.년도='" + thisYear + "' ");
            ...
            ResultSet nursingPriceRS = statement.executeQuery("select A.방문횟수,B.금액 from Su_방문간호정보 as A left join Su_년도별적용급액 as B on B.상세구분=A.총시간이름 where B.년도='" + thisYear + "' AND (A.일자 BETWEEN '" + startMon + "' AND '" + endMon + "') AND A.수급자코드='" + recipiId + "'");
